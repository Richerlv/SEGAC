...
def main():
    ...
    for i_episode in range(100):
        ...
        # 保存模型
        gpg.save_model('/model/gpg_model.pth')

def execute(start, end, time_budget):
    # 初始化地图信息和环境
    map_info = MapInfo("maps/sioux_network.csv")
    env = Env(map_info, start, end)
    env.reset()

    # 初始化模型
    gpg = OffGeneralizedPG(n_node=env.map_info.n_node, time_budget=time_budget, device='cpu')

    # 加载已经训练好的模型权重
    gpg.load_model('./model/gpg_model.pth')
    ...
    for _ in range(1000):
        ...
        mask = env.get_agent_mask()
        mask_tensor = torch.FloatTensor(mask).to(gpg.device)
        action, _ = gpg.select_action(state_tensor.unsqueeze(0), mask_tensor)
        _, _, done = env.step(action)
        if done or len(env.path) > env.map_info.n_node:
            score += 1 if env.cost_time < gpg.time_budget else 0
            break
    print('Score: {}'.format(score / 1000), env.path)
    return env.path

if __name__ == '__main__':
    # main()
    execute(start=2, end=15, time_budget=40)
